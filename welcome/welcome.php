<?php
if(!class_exists('eightmedi_Lite_Welcome')) :

	class eightmedi_Lite_Welcome {
			...
			public function __construct( $plugins, $strings ) {
				...
				$this->tab_sections = array(
					'getting_started' => esc_html__('Getting Started', 'eightmedi-lite'),
					'recommended_plugins' => esc_html__('Recommended Plugins', 'eightmedi-lite'),
					//'actions_required' => esc_html__('Required Plugins', 'eightmedi-lite'),
					'demo_import' => esc_html__('Import Demo', 'eightmedi-lite'),
					'support' => esc_html__('Support', 'eightmedi-lite'),
					'changelog' => esc_html__('Changelog', 'eightmedi-lite'),
					'more_wp' => esc_html__('More WordPress Stuff', 'eightmedi-lite'),
				);
			...
			public function welcome_admin_notice_display(){
				...
				$eightmedi_lite_an_div = '
				<div class="updated eightmedi-lite-an notice notice-success is-dismissible">
				%1$s
				<hr/>
				<div class="eightmedi-lite-column-wrap">
				<div class="eightmedi-lite-column col-img">%2$s</div>
				<div class="eightmedi-lite-column col-demos">%3$s</div>
				<div class="eightmedi-lite-column col-doc">%4$s</div>
				</div> 
				</div>
				<style>%5$s</style>';

				/* translators: 1 - notice title, 2 - notice message */
				$eightmedi_lite_an_top = sprintf(
					'<h2>%1$s</h2><p class="about-desc">%2$s</p></hr>',
					esc_html__( 'Congratulations!', 'eightmedi-lite' ),
					sprintf(
						/* translators: %s - theme name */
						esc_html__( '%s is now installed and ready to use. We\'ve assembled some links to get you started.', 'eightmedi-lite' ),
						$this->theme_name
					)
				);

				$eightmedi_lite_an_btn = sprintf(
					/* translators: 1 - url, 2 - button text */
					'<a href="%1$s" class="button button-primary eightmedi-lite-btn" >%2$s</a>',
					esc_url( admin_url( 'themes.php?page=welcome-page#demo_import' ) ),
					esc_html__( 'Try one of our ready to use Starter Sites', 'eightmedi-lite' )
				);
				$eightmedi_lite_an_pbtn = sprintf(
					...
					esc_html__( 'or go to the theme settings', 'eightmedi-lite' )
				);
				$eightmedi_lite_an_img = sprintf(
					'<picture>
				...
				$eightmedi_lite_an_list = sprintf(
					'<div><h3><span class="dashicons dashicons-images-alt2"></span> %1$s</h3><p>%2$s</p></div><div> <p>%3$s</p><p>%4$s</p> </div>',
					__( 'Sites Library', 'eightmedi-lite' ),
					// translators: %s - theme name
					sprintf( esc_html__( '%s now comes with a sites library with various designs to pick from. Visit our collection of demos that are constantly being added.', 'eightmedi-lite' ), $this->theme_name ),
					$eightmedi_lite_an_btn,
					$eightmedi_lite_an_pbtn
				);
				$eightmedi_lite_an_doc = sprintf(
					'<div><h3><span class="dashicons dashicons-format-aside"></span> %1$s</h3><p>%2$s</p><a class="button eightmedi-lite-btn" href="%3$s">%4$s</a></div><div></div>',
					__( 'Documentation', 'eightmedi-lite' ),
					// translators: %s - theme name
					sprintf( esc_html__( 'Need more details? Please check our full documentation for detailed information on how to use %s.', 'eightmedi-lite' ), $this->theme_name ),
					'https://8degreethemes.com/documentation/eightmedi-lite',
					esc_html__( 'Read full documentation', 'eightmedi-lite' )
				);
				$eightmedi_lite_an_style = '
				.eightmedi-lite-an.notice.is-dismissible {padding: 20px 15px 5px;}
				.eightmedi-lite-an hr {margin: 20px -23px 0;border-top: 1px solid #f3f4f5;border-bottom: none;}
				.eightmedi-lite-an h3{margin: 17px 0 0;font-size: 16px;line-height: 1.4;}
				.eightmedi-lite-an p {color: #72777c;}
				.eightmedi-lite-an h2{margin: 0;font-size: 21px;font-weight: 400;line-height: 1.2;}
				.eightmedi-lite-an p.about-desc{color: #72777c;font-size: 16px;margin: 0;padding:0px;}
				.eightmedi-lite-column-wrap {display: -ms-grid;display: grid;-ms-grid-columns: 24% 32% 32%;grid-template-columns: 24% 32% 32%;margin-bottom: 13px;}
				.eightmedi-lite-column {padding: 20px 40px 0 0;}
				.eightmedi-lite-an .eightmedi-lite-btn {padding: 30px;line-height: 0;}';

				echo sprintf(
					$eightmedi_lite_an_div,
					$eightmedi_lite_an_top,
					$eightmedi_lite_an_img,
					$eightmedi_lite_an_list,
					$eightmedi_lite_an_doc,
					$eightmedi_lite_an_style
				);// WPCS: XSS OK.
				...
				<div class="wrap about-wrap access-wrap">					
					<div class="top-block-wrap">					
						<div class="about-text">
							<h1><?php /* translators: %1$s: theme name, %2$s: theme version */ printf( esc_html__( 'Welcome to %1$s - Version %2$s', 'eightmedi-lite' ), esc_html($this->theme_name), esc_html($this->theme_version) ); ?></h1>
						...
						<div class="welcome-section-wrapper is_loading">
							...
							<div class="notice-sidebar is_loading">
								<div class= "notice-sidebar-item">
									<h4><?php echo esc_html('Join in our social networks!','eightmedi-lite') ?></h4>
									<p><?php echo esc_html__('Get connected, share your opinions and more via our social community:', 'eightmedi-lite'); ?></p>
									<p><a href="<?php echo esc_url('https://www.facebook.com/8DegreeThemes');?>"><?php echo esc_html('Join our Facebook Group','eightmedi-lite') ?></a> <?php echo esc_html('- to receive updates, offers and more.','eightmedi-lite') ?></p>
									<p><a href="<?php echo esc_url('https://www.youtube.com/8degreethemes');?>"><?php echo esc_html('Subscribe our YouTube Channel','eightmedi-lite') ?></a> <?php echo esc_html('- for tutorials, videos and more.','eightmedi-lite') ?></p>
									<p><a href="<?php echo esc_url('https://www.twitter.com/8degreethemes');?>"><?php echo esc_html('Follow us on Twitter','eightmedi-lite') ?></a> <?php echo esc_html('- to stay updated.','eightmedi-lite') ?></p>
								</div>
								<div class= "notice-sidebar-item">
									<h4><?php echo esc_html('Leave us a review','eightmedi-lite') ?></h4>
									<p><?php echo esc_html__('Are you are enjoying eightmedi Lite? We would love to hear your feedback.', 'eightmedi-lite'); ?></p>
									<a href="<?php echo esc_url('https://wordpress.org/support/theme/eightmedi-lite/');?>"><?php echo esc_html('Submit a review','eightmedi-lite') ?></a>
			...
			public function plugin_installer_callback(){
				...
				if ( ! current_user_can('install_plugins') ) {
					wp_die( esc_html__( 'Sorry, you are not allowed to install plugins on this site.', 'eightmedi-lite' ) );
				}
				...
				if (! wp_verify_nonce( $nonce, 'plugin_installer_nonce' )) {
					wp_die( esc_html__( 'Error - unable to verify nonce, please try again.', 'eightmedi-lite') );
				}
			...
			public function plugin_activation_callback() {
				...
				if(file_exists($plugin_file)) {
					...
					echo esc_html__('Plugin Does not Exists' , 'eightmedi-lite');
				}
